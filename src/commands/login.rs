//! `producthunt login`. Authenticates with an Apify API token, opening the console
//! in the browser if interactive, verifying against `/users/me`, and storing the token in the OS keystore.
...
pub fn run(args: LoginArgs) -> Result<()> {
        ...
        && !force
    {
        return Err(Error::invalid(format!("An account named '{existing}' already exists."))
            .fix(format!("Use --force to replace its token: producthunt login {existing} --force")));
    }
    ...
    let (mut ident, mut user_id) = (String::new(), String::new());
    if !no_verify {
        let me = Client::new(&key).me()?;
        ident = identity::describe(&me);
        user_id = identity::user_id(&me);
    }

    {
        ...
        config.accounts.insert(
            name.clone(),
            AccountConfig { identity: ident.clone(), user_id: user_id.clone(), added_at: config::now_utc() },
        );
        config::save(&config)?;
    }
    ...
    print(obj! {
        ...
        "nextStep" => format!("producthunt search \"AI\" --account {name}"),
    });
    Ok(())
}
...
fn prompt_login_key(name: &str, no_browser: bool) -> Result<String> {
    if !std::io::stdin().is_terminal() {
        return Err(Error::invalid("No API token given and no terminal to prompt on.")
            .fix(format!("pbpaste | producthunt login {name} --api-key-stdin")));
    }
